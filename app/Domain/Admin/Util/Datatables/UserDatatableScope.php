...
class UserDatatableScope extends UserBaseDatatableScope
{
    ...
    public function query()
    {
        ...
        if (request()->get('app_id')) {
            $query = Appuser::select('appusers.*')->with(['application', 'application.plateforms'])->where('app_id', request()->get('app_id'))->whereNotNull('appusers.deleted_at'));
        } else {
            $query = Appuser::select('appusers.*')->with(['application', 'application.plateforms'])->whereNotNull('appusers.deleted_at');
        }
